...
if TYPE_CHECKING:
    _LeaveListBase = ListView[Leave]
    _LeaveCreateBase = CreateView[Leave, LeaveForm]
else:
...
class LeaveListView(LoginRequiredMixin, _LeaveListBase):
    ...
    def get_queryset(self) -> QuerySet[Leave]:
        ...
        accessible_pks = [
            leave.pk for leave in qs if has_object_permission("access_person", self.request.user, leave.person)
        ]
        return qs.filter(pk__in=accessible_pks)

    def _people_for_matrix(self, undertaking_id: int | None, first_day: datetime, last_day: datetime) -> list[Person]:
...
class LeaveCreateView(LoginRequiredMixin, _LeaveCreateBase):
    ...
    def form_invalid(self, form: LeaveForm) -> HttpResponseRedirect:
        """Show errors and redirect back to the leave list."""
        messages.error(self.request, str(form.errors))
        return redirect("leave-list")

    def form_valid(self, form: LeaveForm) -> HttpResponseRedirect:
