...
A `Person` is anyone who does work in the system — your own staff or a vendor
consultant. An `Engagement` is a **time-bounded work arrangement** for that
...
- **Delete engagement.** `/engagements/<id>/delete/` (intermediate confirmation)
...
3. Pick the **order** the engagement is under.
4. Set the **start date** and **end date**. The system enforces
   `start_date ≤ end_date`.
5. Set the **FTE** — a decimal between `0` and `1`. `1.0` is full-time,
...
lets the dashboard aggregate by undertaking.
...
for the days on which an order-version assignment exists.

1. Open the engagement's detail page at `/engagements/<id>/`.
2. In the **Order-version assignments** related block, click **Add assignment**.
3. Pick the **order version**. The version must belong to the same
...
## What happens when you edit engagement dates
...
**automatically trimmed** to stay inside it. You will see the adjusted dates
...
|---|---|---|
| "FTE must be between 0 and 1." | The FTE field is outside the allowed range. | Enter a decimal value between 0 and 1. |
| "Order version does not belong to this engagement's order." | You picked a version from a different order. | Pick a version whose order matches the engagement's order. |
| "Undertaking assignment must fall inside the engagement's date range." | The assignment starts before or ends after the engagement. | Move the assignment dates, or extend the engagement first. |
| "Undertaking assignment overlaps another for the same undertaking." | Two assignments of the same `(engagement, undertaking)` overlap. | Close one before opening the next, or edit one to end earlier. |
